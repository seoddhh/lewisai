...
| 진입 방식 | 입력 | 동작 |
|---|---|---|
| **칩 진입 (기본)** | 프론트 위저드가 고른 칩(동반·목적·위치·시간·끼니·인원·일수·밀도) **+ 마지막 단계의 자유서술 `note`** | `req` 가 이미 주입돼 있으므로 `parse_intent` 는 무동작 — 결정적 |
| 자연어 진입 (보조) | `message` 문자열만 | `parse_intent` 가 LLM 으로 문장에서 칩과 같은 구조(시간창·동반·목적·끼니·일수·밀도)를 추출해 합성 |

### 입력 순서: 칩이 먼저, 자유서술(`note`)이 마지막

**서울로 위저드는 칩 선택이 먼저고, 자유서술이 마지막 단계다.** 자연어로 시작해 칩을 추론하는
구조가 아니다 — 사용자는 단계별로 칩을 다 고른 뒤, 마지막 화면에서 "덧붙일 말"을 자유롭게 적는다.
그래서 `note` 는 **칩을 대체하거나 덮어쓰지 않고, 확정된 칩 위에 얹히는 보정 신호**다.

```
위저드:  동반 → 목적 → 위치 → 시간 → 끼니 → 일수·밀도 →  [자유서술 note]  → 생성
         └───────────────── 구조(칩) 확정 ─────────────────┘   └ 마지막에 얹힘 ┘

요청:    { chips: {...확정된 칩...}, note: "야경 보면서 데이트" }
```

`note` 가 파이프라인에서 실제로 쓰이는 지점은 세 곳이고, **권역·시간·끼니 같은 골격은 절대 못 바꾼다**:

| 단계 | `note` 의 영향 | 강도 |
|---|---|---|
| `retrieve` 검색 쿼리 | 목적 확장어·동반·위치·시간과 **이어 붙인 한 문자열**로 임베딩된다 (`note + purpose.query + companions + locations + time`). 가중치는 없다. | △ 후보 30~60% 교체 (실측) |
| `retrieve` 권역 메타필터 | **영향 없음.** `area` 는 칩에서만 오고 Chroma 하드필터로 걸린다 — `note` 에 "한강"이라 써도 위치 칩이 종로·중구면 종로 밖은 검색조차 되지 않는다. | ✕ |
| `select_places` 프롬프트 | `요청: "{note}"` 로 원문이 그대로 들어가 `reason`·`activities`·장소 취향에 반영된다. | ○ |
| `compose` 프롬프트 | 제목·부제·설명의 강조점에 반영된다. | ○ |
| `plan` / `fit_schedule` / `meals` | **영향 없음.** 시간 골격·끼니 시각·방문 순서는 칩과 순수 계산만으로 결정된다. | ✕ |

즉 **`note` 는 "칩이 정한 범위 안에서 무엇을 고를지"를 미세조정한다.** 범위 자체(권역·시간창·끼니·일수)를
바꾸고 싶으면 칩을 바꿔야 한다. `note` 가 칩 범위로 불가능한 요구를 하면(예: 위치=종로·중구 + `note`="한강에서
자전거") 서버는 가장 가까운 근사치를 고른다(청계천 등).

> **알려진 이슈:** 이때 `compose` 가 못 들어준 요구를 제목·부제에 노출하는 경우가 있다
> (실측 5회 중 2회: `"한강 자전거 대신, 종로·중구 연인과 문화 데이트 코스"`). `note` 원문이
> `요청:` 으로 프롬프트에 들어가는데 "충족하지 못한 요구는 언급하지 말라"는 규칙이 없어서다.
> 프롬프트에 규칙 한 줄을 추가하면 막힌다 — 미적용.

예시로 **칩 없는 자연어 단독 진입**(보조 경로) 하나가 실제로 어떻게 처리되는지:
